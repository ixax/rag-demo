...
- `mcp-server` — see [`services/mcp_server/src/server.py`](./services/mcp_server/src/server.py) module docstring for the retrieve → rerank → generate pipeline. `server.py` is handlers + wiring only -- config schema, retrieval, and timing/answer-parsing helpers live in [`services/mcp_server/src/libs/`](./services/mcp_server/src/libs); the AI gateway client classes live in `_common/clients/` (shared with `ingest`). All of these take what they need as arguments rather than reading env/config themselves. Full per-step breakdown of the retrieval pipeline (`RagPipeline.STEP_ORDER`, `services/mcp_server/src/libs/pipeline.py`): README's [Pipeline steps](./README.md#pipeline-steps) section -- read that, not just this file, before reasoning about retrieval/rerank/generation behavior.
